...
try:
    ...
    driver.find_element_by_xpath(f"//select[@name='p_term']/option[text()={term}]").click() # get term
    driver.find_element_by_xpath("//input[@value='Submit']").click() # submit
    driver.find_element_by_xpath(f"//select[@name='sel_subj']/option[text()={major}]").click() # select major
    ...
    courses = [course for course in courses if not any(substring in course for substring in blacklist)] # apply blacklist
